...
export default function Catalog({ initialCategory = '', initialAnimal = '' }) {
  ...
  const filtered = useMemo(() => {
    return PRODUCTS.filter((p) => {
      ...
      if (animal && !p.tags.includes(animal)) return false
      if (query) {
      ...
      <section className="border-b border-limestone-dark bg-paper">
        <div className="mx-auto max-w-[1200px] px-5 lg:px-16 py-12 lg:py-16">
          ...
          <p className="text-[14px] text-slate max-w-[60ch]">
            Full listing of feed raw materials, minerals, and additives. Filter by category, animal type, or product name.
          </p>
        </div>
      </section>
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map((c) => (
                  <button
                    ...
                    className={pillCls(category === c.id, 'bg-anchor-navy')}
                  >
                    {c.name}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {ANIMAL_TAGS.map((t) => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => setAnimal(animal === t ? '' : t)}
                    className={pillCls(animal === t, 'bg-pasture-green')}
                  >
                    {t}
                  </button>
                ))}
      ...
      <section className="bg-paper min-h-[400px]">
        <div className="mx-auto max-w-[1200px] px-5 lg:px-16 py-10">
          <div className="flex items-center justify-between mb-6 text-[13px] text-slate">
            <span>
              <strong className="text-ink font-semibold">{filtered.length}</strong> products shown
              {(category || animal || query) && (
                <button
                  type="button"
                  onClick={reset}
          ...
          {filtered.length === 0 ? (
            <div className="py-16 text-center border border-limestone-dark rounded-sm bg-white">
              <p className="text-slate text-[15px] mb-4">No products match your search.</p>
              <button
                type="button"
                onClick={reset}
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <div className="mt-4 pt-3 border-t border-limestone-dark flex items-center justify-between">
                      <span className="text-[12px] text-slate">
                        Packaging: <strong className="text-ink">{p.packaging}</strong>
                      </span>
                      <Button as="a" href={`#/products/${p.slug}`} variant="tertiary">
                        Details
                        <ArrowRight size={13} />
                      </Button>
                    </div>
